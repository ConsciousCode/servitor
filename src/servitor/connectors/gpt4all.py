...
class GPT4AllCompletion(Completion):
	...
	async def __await__(self):
		async with self.connector.throttle:
...
class GPT4AllConnector(Connector):
	...
	def __init__(self, *, model: Optional[str]=None, model_path: Optional[str]=None, allow_download=True, concurrent=3, **other):
		'''
		Parameters:
			model: Model to use.
		'''
		...
		self.throttle = asyncio.Semaphore(concurrent)
