	...
	* Tokens per period
	...
	async def __call__(self, tokens):
		if self.concurrent.locked():
			logger.info("Throttling", extra={"reason": "concurrent requests"})
		...
		async with self.concurrent:
			while True:
				...
				if self.token_allowance < tokens:
					logger.info("Throttling", extra={"reason": "too many tokens"})
				if self.request_allowance < 1:
					logger.info("Throttling", extra={"reason": "too many request"})
